...
import { StyleSheet, TouchableOpacity, GestureResponderEvent, Platform } from 'react-native'
import { MaterialIcons, AntDesign } from '@expo/vector-icons'
import { Formik, FormikHelpers } from 'formik'
import { Form, Input, Item, InputGroup, Button, Label, Text, View } from 'native-base'
...
const validationSchema = object().shape({
  ...
  email: string().email('Email must be a valid email').required('Please, provide your email!'),
  password: string()
...
export default function SignUpForm({ handleUserEmail: handleConfirmationEmail }: SignUpFormProps) {
  const navigation = useNavigation()

  const onSubmit = async (values: ValueSignUpForm, submitProps: FormikHelpers<ValueSignUpForm>) => {
    const { email, name, password } = values
    await signUp(email, password, name, submitProps)
  }

  async function signUp(email: string, password: string, name: string, submitProps: FormikHelpers<ValueSignUpForm>) {
    try {
      await Auth.signUp({
        username: email,
        password,
        ...
        <View style={styles.wrapper}>
          ...
          >
            {({ values, handleChange, errors, setFieldTouched, touched, isValid, handleSubmit }) => (
              <Form>
                <InputGroup success={isValid} error={!isValid} style={styles.inputGroup}>
