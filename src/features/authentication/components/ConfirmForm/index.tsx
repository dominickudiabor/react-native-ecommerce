...
import { Form, Input, Item, InputGroup, Label, Text, Button } from 'native-base'
import { StyleSheet, TouchableOpacity, GestureResponderEvent, SafeAreaView } from 'react-native'
...
const validationSchema = object().shape({
  code: string().length(6, 'Code should be exactly 6 digits long').required('Please, provide your confirmation code!'),
})

export default function ConfirmForm({ userEmail, handleUserEmail }: ConfirmFormProps) {
  const navigation = useNavigation()

  const onSubmit = async (values: ValueConfirmForm, submitProps: FormikHelpers<ValueConfirmForm>) => {
    await confirmEmail(userEmail, values.code, submitProps)
  }

  async function confirmEmail(userEmail: string, code: string, submitProps: FormikHelpers<ValueConfirmForm>) {
    try {
  ...
  return (
    <KeyboardAwareScrollView resetScrollToCoords={{ x: 0, y: 0 }} contentContainerStyle={styles.wrapper} scrollEnabled>
      <SafeAreaView>
        ...
        >
          {({ values, handleChange, errors, setFieldTouched, touched, isValid, handleSubmit }) => (
            <Form style={styles.formContainer}>
              ...
              <InputGroup style={styles.wrapperInputs}>
